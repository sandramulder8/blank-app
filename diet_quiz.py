...
def quiz():
    ...
    "Fast800": 0, "TLC": 0, "Mayo Clinic": 0, "Flexitarian": 0, "Sirtfood": 0, "South Beach": 0, "HMR": 0, "Dukan": 0, "PSMF": 0, "ADF": 0, "Military": 0, "CICO": 0, "Vegan": 0}

    q1 = st.radio("Do you prefer structured meal plans or flexibility?", 
                  ("Structured", "Flexible", "A mix of both"))
    if q1 == "Structured":
        score["Keto"] += 1
        score["WW"] += 1
        score["Fast800"] += 1
    ...
    q2 = st.radio("How fast do you want to lose weight?", 
                  ("Fast results (1-2 kg per week / 2+ pounds per week)", "Steady long-term weight loss (0.5-1 kg per week / 1-2 pounds per week)", "I want long-term weight loss as part of a healthy lifestyle"))
    if q2 == "Fast results (1-2 kg per week / 2+ pounds per week)":
        score["Keto"] += 1
        score["Military"] += 1
        score["Fast800"] += 1
        score["ADF"] += 1
    elif q2 == "Steady long-term weight loss (0.5-1 kg per week / 1-2 pounds per week)":
        score["HMR"] += 1
        score["PSMF"] += 1
    else:
        score["Mediterranean"] += 1
    ...
    if st.button("Get My Recommended Diet!"):
        ...
        diet_links = {
            ...
            "Vegan": "[Learn more about Vegan Diet](https://yourwebsite.com/vegan)"
        }
